...
doubledouble sqrt(const doubledouble& y) {
  unsigned short cw = x86_fix_start();
...
doubledouble sin(const doubledouble& x) {
  ...
  t2-=a;
  t2*=8;
  int b=int(rint(t2));   // must have |a|<=2 and |b|<=7 now
  doubledouble s=k3;
  s+=k3;
  s-=(8*a+b)/16.0;
  s*=doubledouble::Pi; // s is now reduced argument. -Pi/32 < s < Pi/32
  doubledouble s2=sqr(s); 
  ...
  sins+=sinsTab[0];
  sins*=s;
  doubledouble coss=1.0;
  coss-=sqr(sins);
  coss=sqrt(coss);  // ok as -Pi/32 < s < Pi/32
  // here sinb=sin(b*Pi/16) etc.
  if (b >= 0) {
    doubledouble sinb = tab[b];
    doubledouble cosb = tab[8-b];
    if (0==a) {
      sins*=cosb;
      coss*=sinb;
      return  sins+=coss;
...
// cos
doubledouble cos(const doubledouble& x) {
  doubledouble d=doubledouble::Pion2;
  d-=x;
  return sin(d);
}
